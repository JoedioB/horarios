...
class GeradorHorarioService {
    async gerar() {
        const [professores, turmas, gruposGeminados] = await Promise.all([
            models_1.Professor.findAll({ include: [{ model: models_1.Disciplina, as: 'disciplinas' }, { model: models_1.DisponibilidadeProfessor, as: 'disponibilidades' }] }),
            models_1.Turma.findAll({ include: [{ model: models_1.Disciplina, as: 'Disciplinas' }] }),
            models_1.GrupoGeminado.findAll({ include: [{ model: models_1.Turma, as: 'turmas' }, models_1.Disciplina, { model: models_1.Professor, include: [{ model: models_1.DisponibilidadeProfessor, as: 'disponibilidades' }] }] })
        ]);
        const horarioTurmas = {};
        ...
        try {
            await models_1.Horario.destroy({ where: {}, truncate: false, cascade: false, transaction: t });
            if (novosHorarios.length > 0) {
                await models_1.Horario.bulkCreate(novosHorarios, { transaction: t });
            }
            await t.commit();
    ...
    async _alocarRegrasFixas(turmas, todosProfessores, horarioTurmas, horarioProfessores, novosHorarios) {
        ...
        const diaTerça = 2;
        const slotsIniciais = [constants_1.SLOTS[0], constants_1.SLOTS[1]];
        ...
        for (const turma of turmasAlvo) {
            ...
            if (optativa) {
                ...
                const professor = todosProfessores.find(p => p.disciplinas.some((d) => d.id === optativa.id));
                if (professor) {
    ...
    _prepararTarefasPriorizadas(turmas, gruposGeminados, todosProfessores, horariosJaAlocados) {
        ...
        gruposGeminados.forEach(grupo => {
            ...
            for (let i = 0; i < duplas; i++)
                tarefas.push({ tipo: 'geminada', entidade: grupo, tipoBloco: 2, prioridade: 1, alocada: false });
            for (let i = 0; i < simples; i++)
                tarefas.push({ tipo: 'geminada', entidade: grupo, tipoBloco: 1, prioridade: 3, alocada: false });
        });
        turmas.forEach(turma => {
            turma.Disciplinas.forEach((disciplina) => {
                if (!turmasEmGrupos.has(`${turma.id}-${disciplina.id}`)) {
                    ...
                    const simples = cargaRestante % 2;
                    for (let i = 0; i < duplas; i++)
                        tarefas.push({ tipo: 'normal', entidade: { turma, disciplina, professoresQualificados }, tipoBloco: 2, prioridade: 2, alocada: false });
                    for (let i = 0; i < simples; i++)
                        tarefas.push({ tipo: 'normal', entidade: { turma, disciplina, professoresQualificados }, tipoBloco: 1, prioridade: 4, alocada: false });
                }
            });
        });
    ...
    async _tentarAlocarTarefa(tarefa, horarioTurmas, horarioProfessores, novosHorarios) {
        const slotsParaTentar = tarefa.tipoBloco === 2 ? constants_1.SLOTS_PARES : constants_1.SLOTS.map(s => ({ slot1: s }));
        ...
        for (const dia of dias) {
            for (const par of slotsEmbaralhados) {
                ...
                if (!slots.every(s => (0, constants_1.isSlotValidoParaDia)(dia, s.inicio))) {
                    continue;
                }
    ...
    _isProfessorDisponivel(professor, dia, horaInicioSlot) {
        if (!professor || !professor.disponibilidades)
            return false;
        const horaSlot = (0, moment_1.default)(horaInicioSlot, 'HH:mm:ss');
        for (const disp of professor.disponibilidades) {
            if (disp.dia_semana === dia) {
                const inicioDisp = (0, moment_1.default)(disp.hora_inicio, 'HH:mm:ss');
                const fimDisp = (0, moment_1.default)(disp.hora_fim, 'HH:mm:ss');
                if (horaSlot.isSameOrAfter(inicioDisp) && horaSlot.isBefore(fimDisp))
                    return true;
            }
        }
        return false;
...
exports.default = new GeradorHorarioService();
